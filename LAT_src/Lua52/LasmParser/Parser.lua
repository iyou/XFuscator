local Parser = {
    new = function(self, source, name)
        return setmetatable({ 
            lexed = source and (type(source) == "table" and source) or (type(source) == "string" and LAT.Lua52.Lexer:new():Lex(source, "<unknown>")) or nil,
            name = name
        }, { __index = self })
    end,
    ...
    Parse = function(self, source, name)
        ...
        local tok = self.lexed or (type(source) == "table" and source) or (type(source) == "string" and LAT.Lua52.Lexer:new():Lex(source, name or "<unknown>"))
        ...
        local function isOpcode(d)
            local x,y = pcall(function() LAT.Lua52.Instruction:new(d) end)
            --print(x, y)
            return x
        end
        ...
        local function addConst(alwaysDefine)
            ...
            if alwaysDefine or constTable[value] == nil then
                if value == true or value == false then
                    func.Constants:Add(LAT.Lua52.Constant:new("Bool", value))
                elseif value == nil then
                    func.Constants:Add(LAT.Lua52.Constant:new("Nil", nil))
                elseif type(value) == "number" then
                    func.Constants:Add(LAT.Lua52.Constant:new("Number", value))
                elseif type(value) == "string" then
                    func.Constants:Add(LAT.Lua52.Constant:new("String", value))
                end
        ...
        local function doControl()
            ...
            elseif tok:ConsumeKeyword".local" then
                ...
                func.Locals:Add(LAT.Lua52.Local:new(name, 0, 0))
            elseif tok:ConsumeKeyword".upval" or tok:ConsumeKeyword".upvalue" then
                ...
                local instack = readNum()
                local index = readNum()
                func.Upvalues:Add(LAT.Lua52.Upvalue:new(instack, index, name))
            ...
            elseif tok:ConsumeKeyword".end" then
                ...
                local instr2 = LAT.Lua52.Instruction:new("RETURN")
                instr2.A = 0
                instr2.B = 1
                instr2.C = 0
                if instr1 then
        ...
        local function parseOpcode()
            ...
            local instr = LAT.Lua52.Instruction:new(op, 0)
